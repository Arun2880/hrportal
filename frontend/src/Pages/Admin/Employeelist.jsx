...
const Employeelist = () => {
  ...
  const GetEmployeelist = async () => {
    try {
      const url = 'http://hrportal.dreambytesolution.com/admin/emplist';
      const headers = {
  ...
  const handleDeleteClick = (employeeId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const url = `http://hrportal.dreambytesolution.com/admin/delemp/${employeeId}`;
          const headers = {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          };
  ...
  const handleUpdateEmployee = async () => {
    try {
      const url = `http://hrportal.dreambytesolution.com/admin/updateemp/${currentEmployee._id}`;
      const headers = {
      ...
      if (currentEmployee.adhar) formData.append('adhar', currentEmployee.adhar);
      if (currentEmployee.pan) formData.append('pan', currentEmployee.pan);
      if (currentEmployee.resume) formData.append('resume', currentEmployee.resume);
      if (currentEmployee.experience_letter) formData.append('experience_letter', currentEmployee.experience_letter);
  ...
  const columns = [
    ...
    { field: 'createdAt', headerName: 'Date', width: 150 },
    {
      ...
      renderCell: (params) => (
        <img
          src={`http://hrportal.dreambytesolution.com/${params.value}`}
          alt="Aadhar"
          style={{ width: 50, height: 50, cursor: 'pointer' }}
          onClick={() => handleImageClick(`http://hrportal.dreambytesolution.com/${params.value}`)}
        />
      ),
    },
    {
